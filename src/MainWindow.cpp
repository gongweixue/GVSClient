...
void MainWindow::showOrientationMarker() {
    ...
    vtkSmartPointer<GVSCompassActor> gvsCompass = 
        vtkSmartPointer<GVSCompassActor>::New();
    ...
    m_OrientationMarker->SetOutlineColor(backColor[0], 
                                         backColor[1], 
                                         backColor[2]);
}
...
void MainWindow::RenderStdExplode() {
    ...
    for (int i = 0; i < (m_evenExplodeRow - 1); i++) {
        ...
        int originY = (bounds[3] - (i + 1) * (bounds[3] - bounds[2]) 
            / m_evenExplodeRow);
        hPlane[i]->SetOrigin(0, originY, 0);
...
void MainWindow::RenderBoxClip() {
    ...
    m_boxClipWidget->AddObserver(vtkCommand::EndInteractionEvent, 
                                 m_boxClipCallback);
    //insert record
    vtkActor* tmpActor = MapToActor(apdResult->GetOutput());
    m_sceneManager.InsertActorRcrd(tmpActor, 
...
void MainWindow::OnEditColorLegend() {
    ColorLegendEditor* editor = 
        new ColorLegendEditor(m_ColorLegendManager, this);
    editor->exec();
    delete editor;
}
...
QTreeWidgetItem* MainWindow::findObjItemInPrjTree(const QString& modelName,
    ...
    QList<QTreeWidgetItem*> modelList = 
        m_prjTreeWidget->findItems(modelName, Qt::MatchCaseSensitive | Qt::MatchExactly);
...
void MainWindow::prjExplorerFavGroupChanged(GVSPrjTreeWidgetItem* item_clicked) {
    ...
    Qt::CheckState newGroupState = item_clicked->checkState(0);
...
void MainWindow::OnChangingObjColor(const QString& modelName, const QString& objName) {
    ...
    if (m_pDoc->GetObjManager()->getObjColor(modelName, objName, oldRGB)) {
        ...
        //QMessageBox::warning(NULL, tr("出错"), tr("无法得到对象原始颜色！"));
    }
}
            ...
            for (int i = 0; i < favRootItem->childCount(); ++i) {
                ...
                if (0 == newGroupName.compare(group->text(0))) {
                    ...
                    if (obj) {
                        ...
                        newItem->setIcon(0, QIcon(":/Resources/PrjExplorer/NoObj.png"));
